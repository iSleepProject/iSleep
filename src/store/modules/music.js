...
export default defineStore("music", {
  ...
  actions: {
    ...
    order_play_list(id = "01") {
      ...
      if (index === 0) {
        const music = middleware.shift();
        return {
          _music: music,
          middleware,
        };
      } else if (index === middleware.length - 1) {
        const music = middleware.pop();
        return {
          _music: music,
          middleware,
        };
    ...
    random_play_list(id = "01") {
      ...
      const middleware = JSON.parse(JSON.stringify(this.original_list));
      const index = middleware.findIndex((i) => i.id === id_str);
      const [music] = middleware.splice(index, 1);
      const output = [];
    ...
    play(id) {
      ...
      this.music_title = music.title;
      this.music_singer = music.singer;
      this.music_coverImgUrl =
        music.pic_src ||
        "https://636c-cloud1-9gwoav2d1358e3bc-1312093740.tcb.qcloud.la/users_img/logo.png?sign=b806e62233162987f65bc8c4441c7daf&t=1656501899";
      this.music_src = music.src;
      this.is_love = music.is_love;
    ...
    next() {
      ...
      const music = this.play_list.shift();
      this.music_id = music.id;
      this.music_title = music.title;
      this.music_singer = music.singer;
      this.music_src = music.src;
    ...
    previous() {
      ...
      const music = this.have_played_list.pop();
      this.music_id = music.id;
      this.music_title = music.title;
      this.music_singer = music.singer;
      this.music_src = music.src;
